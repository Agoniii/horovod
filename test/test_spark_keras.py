...
def test_model_serialization(mock_remote_trainer):
    model = create_xor_model()
    optimizer = tf.keras.optimizers.SGD(lr=0.1)
    loss = 'binary_crossentropy'
...
def test_serialize_param_value():
...
def test_calculate_shuffle_buffer_size():
...
def test_custom_sparse_to_dense_fn():
    ...
    sess = tf.Session()
    sparse_vector_values = sess.run(sparse_vector)[0]
...
def test_convert_custom_sparse_to_dense_bare_keras_fn():
...
def test_prepare_data_bare_keras_fn():
    metadata = \
        {
            'col1': {
                'dtype': float,
...
def test_reshape():
    ...
    sess = tf.Session()
    reshaped_row_value = sess.run(reshaped_row)
    sess.close()
...
def test_prep_data_tf_keras_fn_with_sparse_col():
    has_sparse_col = True
    ...
    sess = tf.Session()
    prepped_row_vals = sess.run(prepped_row)
    sess.close()
...
def test_prep_data_tf_keras_fn_without_sparse_col():
